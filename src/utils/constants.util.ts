...
import { RepositoryI as ElasticsearchDataLayer } from '@localrepo/lib_data_access_elasticsearch';
import { RepositoryI as MongoDataLayer } from '@localrepo/lib_data_access_mongodb';
...
export const getStatusTexts = (statusCode: number): string => {
...
export interface makeResponseI {
  code: number;
  message: string;
  data: any;
  timestamp: number;
...
export const apiResponse = (res: Response, data: ResponseI): any => {
  return res.status(data.statusCode).json({
    code: data.statusCode,
    message: getStatusTexts(data.statusCode),
    data: data,
    timestamp: Date.now(),
  });
};
...
export interface DbClients {
  elastic: ElasticsearchDataLayer;
